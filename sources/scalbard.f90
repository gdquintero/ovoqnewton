 Program scalbard
    ...
    real(kind=8) :: ui,vi,wi,di,ytrue,p1,p2,ratio,ratio_best,rperturb
    ...
    ! ------------------------------------------------------------------
    ! Read algorithm parameters: delta, sigmin, gamma, cauchy_flag
    ! ------------------------------------------------------------------
    Open(Unit = 100, file = 'param.txt')
    read(100,*) delta,sigmin,gamma,cauchy_flag
    close(100)

    n = 4                 ! 3 model parameters + 1 epigraph variable w
    ...
    ! ------------------------------------------------------------------
    ! Read the synthetic data generated by data_scalbard.py.
    ! First line: "m obar" (number of observations and of generated outliers).
    ! ------------------------------------------------------------------
    Open(Unit = 100, File = trim(pwd)//"/../data/scalbard.txt", ACCESS = "SEQUENTIAL")
    read(100,*) samples, obar
    ndata = samples

    allocate(t(samples),y(samples),x(n),xk(n-1),xbest(n-1),xtrial(n-1),l(n),u(n),xinit(n-1),faux(samples),&
    ...
    ! positive (same box as the m=19 Bard experiment of Section 4.2).
    l(1) = -10.0d0;  u(1) = 10.0d0
    l(2) =   0.1d0;  u(2) = 10.0d0
    l(3) =   0.1d0;  u(3) = 10.0d0
    l(n) = -1.0d+20; u(n) =  0.0d0

    ! ------------------------------------------------------------------
    ! Starting point: box-constrained least-squares fit to the (contaminated)
    ! data, computed for each m by least_squares.py. The box x2,x3 in [0.1,10]
    ! excludes Bard's spurious minimum, so the fit stays bounded (an
    ! unconstrained fit would diverge to (.,+-inf,+-inf)).
    ! ------------------------------------------------------------------
    Open(Unit = 100, File = trim(pwd)//"/../output/sol_ls_scalbard.txt", ACCESS = "SEQUENTIAL")
    read(100,*) xinit(1)
    read(100,*) xinit(2)
    read(100,*) xinit(3)
    close(100)
    do i = 1, n-1
        xinit(i) = max(l(i), min(u(i), xinit(i)))
    end do

    write(*,"('# ',A2,' m=',I8,' obar=',I7,'  LS start = (',3(F10.5,1X),')')") tag, samples, obar, xinit(1),xinit(2),xinit(3)
    ...
    Open(Unit = 97, File = trim(pwd)//"/../output/scalbard_"//trim(tag)//"_curve.txt", ACCESS = "SEQUENTIAL")
    ...
    do o = omin, omax, ostep
        ...
        do itrial = 1, ntrials
            xk(:) = xinit(:)
            do i = 1, n-1
                ! Same perturbation as the original scalability code (andreani_scaled.f90)
                xk(i) = xk(i) + (2.0d0*drand(seed) - 1.0d0) * 5.0d-1 * max(1.0d0, abs(xk(i)))
                xk(i) = max(l(i), min(u(i), xk(i)))       ! project (needed: Bard denominator)
            end do
    ...
    subroutine ovo_algorithm(pdata,q,noutliers,t,y,indices,Idelta,samples,m,n,xtrial, &
        ...
        real(kind=8)        :: terminate,alpha,epsilon,lambda_min,aux_iden,lambda_max,ri
    ...
    subroutine model(x,i,n,t,samples,res)
        ...
        ! Generalized Bard model: x1 + u/(v*x2 + w*x3), u=t, v=16-t, w=min(u,v)
        ui = t(i)
        vi = 16.0d0 - t(i)
        wi = min(ui,vi)
